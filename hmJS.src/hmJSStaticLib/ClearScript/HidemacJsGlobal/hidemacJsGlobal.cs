...
{
    internal sealed partial class hidemacJsGlobal
    {
        public static string GetHidemacJsGlobalText()
        {
            return @"
    ...
    (function () {
        ...
        getVar = function (s) { return hm.Macro.Var(s); }; hidemaru.getVar = getVar;
        setVar = function (s, v) { return hm.Macro.Var(s, v); }; hidemaru.setVar = setVar;
        evalMacro = function (s) { return hm.Macro.Eval(s); }; hidemaru.evalMacro = evalMacro;
        evalJs = function (s) { return eval(s); };

        //1s 1s2s 数値パラメータを強制文字列化
        //2rn 3rn オブジェクト参照数値返し
        //Ary オブジェクト[]展開
        var hidemacJsGlobalStatements = ""var s=m+' ';for(var i=0;i<arguments.length;i++){if(i>0)s+=',';var a=arguments[i];if(typeof(a)=='string'){s+='R\""\xFF('+a+')\xFF\""';}else{s+=a;}}evalMacro(s+';###=result;');var r=getVar('###');"";
        var hidemacJsGlobalStatements1s = ""var s=m+' ';for(var i=0;i<arguments.length;i++){if(i>0)s+=',';var a=arguments[i];if(i==0||typeof(a)=='string'){s+='R\""\xFF('+a+')\xFF\""';}else{s+=a;}}evalMacro(s+';###=result;');var r=getVar('###');"";
        var hidemacJsGlobalStatements1s2s = ""var s=m+' ';for(var i=0;i<arguments.length;i++){if(i>0)s+=',';var a=arguments[i];if(i==0||i==1||typeof(a)=='string'){s+='R\""\xFF('+a+')\xFF\""';}else{s+=a;}}evalMacro(s+';###=result;');var r=getVar('###');"";
        var hidemacJsGlobalStatementsAry = ""var s=m+' ';for(var i=0;i<c;i++){if(i>0)s+=',';var a=ary[i];if(typeof(a)=='string'){s+='R\""\xFF('+a+')\xFF\""';}else{s+=a;}}evalMacro(s+';###=result;');var r=getVar('###');"";
        var hidemacJsGlobalFomulaNumber1s = ""var s='###='+m+'(';for(var i=0;i<arguments.length;i++){\if(i>0)s+=',';var a=arguments[i];if(i==0||typeof(a)=='string'){s+='R\""\xFF('+a+')\xFF\""';}else{s+=a;}}s+=')';evalMacro(s+';');var r=getVar('###');"";
        var hidemacJsGlobalFomulaNumber1s2s = ""var s='###='+m+'(';for(var i=0;i<arguments.length;i++){\if(i>0)s+=',';var a=arguments[i];if(i==0||i==1||typeof(a)=='string'){s+='R\""\xFF('+a+')\xFF\""';}else{s+=a;}}s+=')';evalMacro(s+';');var r=getVar('###');"";
        var hidemacJsGlobalFomulaNumber3rn = ""var s='###='+m+'(';for(var i=0;i<arguments.length;i++){\if(i>0)s+=',';var a=arguments[i];if(i==2){s+='###3';}else if(typeof(a)=='string'){s+='R\""\xFF('+a+')\xFF\""';}else{s+=a;}}s+=')';evalMacro(s+';');var r=getVar('###');"";
        var hidemacJsGlobalFomulaNumber = ""var s='###='+m;""
            ...
            + ""evalMacro(s+';');var r=getVar('###');"";	//パラメータなし=キーワード
        var hidemacJsGlobalFomulaString2rn = ""var s='$$$='+m+'(';for(var i=0;i<arguments.length;i++){\if(i>0)s+=',';var a=arguments[i];if(i==1){s+='###2';}else if(typeof(a)=='string'){s+='R\""\xFF('+a+')\xFF\""';}else{s+=a;}}s+=')';evalMacro(s+';');var r=getVar('$$$');"";
        var hidemacJsGlobalFomulaString = ""var s='$$$='+m;""
            + ""if(arguments.length>=1){s+='(';for(var i=0;i<arguments.length;i++){if(i>0)s+=',';var a=arguments[i];if(typeof(a)=='string'){s+='R\""\xFF('+a+')\xFF\""';}else{s+=a;}}s+=')';}""	//パラメータあり=関数
            + ""evalMacro(s+';');var r=getVar('$$$');"";	//パラメータなし=キーワード
        var hidemacJsGlobalFomulaSorN = ""var s=m;""
            + ""if(arguments.length>=1){s+='(';for(var i=0;i<arguments.length;i++){if(i>0)s+=',';var a=arguments[i];if(typeof(a)=='string'){s+='R\""\xFF('+a+')\xFF\""';}else{s+=a;}}s+=')';}""	//パラメータあり=関数
            + ""evalMacro('###='+s+';');var r=getVar('###');if(r==0){evalMacro('$$$='+s+';');var rs=getVar('$$$');if(rs!='0'&&rs!=''){r=rs;}}"";	//文字列""0""を判別不能なのでネイティブ実装したほうがいい
        var HidemacJsGlobalNotSupport = ""var serr='Not supported \""'+m+'\""';hm.Macro.Function(\""message\"", serr);console.log(serr);"";
        ...
        // var co2 = ""var r;if(arguments.length>=2){r=hidemaru.createObject(arguments[0],arguments[1]);}else{r=hidemaru.createObject(arguments[0]);}"";
        ...
        //非サポート yes=1; no=0; true=1; false=0; eof=-1;
        ...
        hg.unicode = function () { var m = ""unicode""; eval(fn); return r; }; if (f) unicode = hg.unicode;	//キーワード/関数 両方
        ...
        //[各種状態関連キーワード]
        hg.overwrite = function () { var m = ""overwrite""; if (arguments.length == 0) { eval(fn); } else { eval(st1s); } return r; }; if (f) overwrite = hg.overwrite;	//パラメータなし=キーワード それ以外=文
        ...
        hg.fontname = function () { var m = ""fontname""; eval(fs); return r; }; if (f) fontname = hg.fontname;		//V918B2 ns->fs
        hg.fontsize = function () { var m = ""fontsize""; eval(fn); return r; }; if (f) fontsize = hg.fontsize;		//V918B2 ns->fn
        //getconfig
        //	fontcharset=""fontcharset""; boldstate=""boldface""; width=""orikaeshi""; kinsokustate=""kinsoku""; correctlineno=""correctlineno""; linespace=""lf""; freecursor=""freecursor""; tabcount=""tab""; indentstate=""indent"";
        //	showtab=""showtab""; showruler=""ruler""; tabruler=""tabruler""; linenostate=""showlineno""; pagestate=""showpageno""; formwidth=""formline"";
        //	hilightstate=""hilight""; hilighttitle=""hilighttitle""; savewitheof=""savewitheof""; ignoreeof=""ignoreeof""; backup=""backup"";
        //getconfigcolor
        ...
        //[アウトライン関連キーワード]
        hg.foldable = function () { var m = ""foldable""; eval(fn); return r; }; if (f) foldable = hg.foldable;	//キーワード/文 両方
        ...
        hg.wcs_to_char = function () { var m = ""wcs_to_char""; eval(fn); return r; }; if (f) wcs_to_char = hg.wcs_to_char;	//wideindex_to_charindex
        hg.char_to_wcs = function () { var m = ""char_to_wcs""; eval(fn); return r; }; if (f) char_to_wcs = hg.char_to_wcs;	//charindex_to_wideindex
        ...
        hg.getlinecount = function () { var m = ""getlinecount""; eval(fn3rn); arguments[2].column = getVar(""###3""); return r; }; if (f) getlinecount = hg.getlinecount;
        //第3パラメータはオブジェクトを渡し、オブジェクトのcolumnメンバに返る。
        //mac : #cLine=getlinecount(""ABCD\nXYZ"",7,#cColumn);
        //js : refobj={}; cLine=getlinecount(""ABCD\nXYZ"",7,refobj); cColumn=refobj.column;
        ...
        hg.getresultex = function () { var m = ""getresultex""; eval(fsn); return r; }; if (f) getresultex = hg.getresultex;	//文字列で固定->V918B3 自動
        ...
        //[ファイル系文]
        //新しい秀丸エディタ起動は非推奨
        //パラメータの予約語は非サポートで、数値を指定
        //	sjis=0x01; unicode=0x02; unicode_be=0x07; euc=0x03; jis=0x04; utf7=0x05; utf8=0x06; euro=0x08; gb2312=0x09; big5=0x0A; euckr=0x0B; johab=0x0C; easteuro=0x0D; baltic=0x0E; greek=0x0F; russian=0x10; symbol=0x11; turkish=0x12; hebrew=0x13; arabic=0x14; thai=0x15; vietnamese=0x16; mac=0x17; oem=0x18; default=0x19; utf32=0x1B; utf32_be=0x1C;
        //	binary=0x1A; lf=0x40; cr=0x80; bom=0x0600; nobom=0x0400;
        //	noaddhist=0x0100; selection=0x2000; ws=0x0800; wb=0x1000;
        ...
        hg.exit = function () { var m = ""exit""; eval(st); return r; }; if (f) exit = hg.exit;
        hg.exitall = function () { var m = ""exitall""; eval(st); return r; }; if (f) exitall = hg.exitall;
        hg.saveexit = function () { var m = ""saveexit""; eval(st); return r; }; if (f) saveexit = hg.saveexit;
        hg.saveexitall = function () { var m = ""saveexitall""; eval(st); return r; }; if (f) saveexitall = hg.saveexitall;
        hg.quit = function () { var m = ""quit""; eval(st); return r; }; if (f) quit = hg.quit;
        hg.quitall = function () { var m = ""quitall""; eval(st); return r; }; if (f) quitall = hg.quitall;
        ...
        hg.escapeselect = function () { var m = ""escape""; eval(st); return r; }; if (f) escapeselect = hg.escapeselect;	//escapeから変更
        ...
        hg.del = function () { var m = ""delete""; eval(st); return r; }; if (f) del = hg.del;	//deleteから変更
        ...
        hg.toupper = function () { var m = ""toupper""; if (arguments.length >= 1 && typeof (arguments[0]) == ""string"") { eval(fs); } else { eval(st); } return r; }; if (f) toupper = hg.toupper;	//最初のパラメータが文字列=関数 それ以外=文
        hg.tolower = function () { var m = ""tolower""; if (arguments.length >= 1 && typeof (arguments[0]) == ""string"") { eval(fs); } else { eval(st); } return r; }; if (f) tolower = hg.tolower;	//最初のパラメータが文字列=関数 それ以外=文
        ...
        hg.filter = function () { var m = ""filter""; if (arguments.length >= 4) { eval(fs); } else { eval(st); } return r; }; if (f) filter = hg.filter;	//パラメータ4つ=関数 パラメータ3つ=文
        ...
        //[検索系文]
        //パラメータの予約語は非サポートで、searchoption,searchoption2相当の数値を指定
        //	word casesense nocasesense hilight nohilight regular noregular fuzzy
        //	inselect inselect2 incolormarker maskcomment maskifdef masknormal maskscript maskstring masktag maskonly linknext loop noclose ask
        //	subdir icon filelist fullpath outputsingle outputsametab outputpane(元々なし) unmatch backup preview
        hg.find1 = function () { var m = ""find""; eval(st); return r; }; if (f) find1 = hg.find1;	//findから変更
        ...
        hg.replace1 = function () { var m = ""replace""; eval(st); return r; }; if (f) replace1 = hg.replace1;	//replaceから変更
        ...
        hg.split = function () { var m = ""split""; eval(st); return r; }; if (f) split = hg.split;	//split(文だけサポート)
        ...
        hg.wnidowhorz = function () { var m = ""wnidowhorz""; eval(st); return r; }; if (f) wnidowhorz = hg.wnidowhorz;
        ...
        //[メッセージ系文]
        hg.message = function () { var m = ""message""; eval(fn1s2s); return r; }; if (f) message = hg.message;	//関数
        hg.question = function () { var m = ""question""; eval(st1s); return r; }; if (f) question = hg.question;	//文
        ...
        hg.setcompatiblemode = function () { var m = ""setcompatiblemode""; eval(fn); return r; }; if (f) setcompatiblemode = hg.setcompatiblemode;//関数
        ...
        hg.menuarray = function (ary, c) { var m = ""menu""; eval(stary); return r; }; if (f) menuarray = hg.menuarray;	//オブジェクトの[]（Arrayではない）
        hg.mousemenuarray = function (ary, c) { var m = ""mousemenu""; eval(stary); return r; }; if (f) mousemenuarray = hg.mousemenuarray;	//オブジェクトの[]（Arrayではない）
        ...
        hg.enumregvalue = function () { var m = ""enumregvalue""; eval(fs2rn); arguments[1].regtype = getVar(""###2""); return r; }; if (f) enumregvalue = hg.enumregvalue;
        //第2パラメータはオブジェクトを渡し、オブジェクトのregtypeメンバに返る。
        //mac : $s=enumregvalue(#i,#nRegType);
        //js : refobj={}; s=enumregvalue(i,refobj); nRegType=refobj.regtype;
        ...
        hg.getconfig = function () { var m = ""getconfig""; eval(fsn); return r; }; if (f) getconfig = hg.getconfig;//文字列で固定->V918B3 自動
        ...
        hg.hidemaruhandle = function () { var m = ""hidemaruhandle""; eval(fn); return r; }; if (f) hidemaruhandle = hg.hidemaruhandle;//関数
        ...
        hg.getclipboard = function () { var m = ""getclipboard""; eval(fs); return r; }; if (f) getclipboard = hg.getclipboard;//関数
        ...
        hg.getclipboardinfo = function () { var m = ""getclipboardinfo""; eval(fs); return r; }; if (f) getclipboardinfo = hg.getclipboardinfo;
        ...
        hg.geteventparam = function () { var m = ""geteventparam""; eval(fsn); return r; }; if (f) geteventparam = hg.geteventparam;//文字列で固定->V918B3 自動
        ...
        //	dllfunc関係
        //	COM関係
        //	openfile等のオプションは数値限定
        //	searchdown等のオプションは数値限定
        //	getconfig, getconfigcolor関係のキーワード幾つかはgetconfig, getconfigcolorのパラメータで
        //	eval -> hidemaru.evalMacro
        //	escape -> escapeselect
        //	delete -> del
        //	find -> find1
        //	replace -> replace1
        //	getlinecountの第3パラメータはオブジェクト
        //	enumregvalueの第2パラメータはオブジェクト
        //	split(関数) -> xxx=yyy.split("","")
        //	join -> yyy=xxx.join("","")
        //	encodeuri -> encodeURI
        //	decodeuri -> decodeURI
        ...
        //特殊
        hg.execjs = function (file) { try { var t = hidemaru.loadTextFile(file); eval(t); } catch (e) { if (e == 'endmacro') { } else if (e == 'endmacroall') { throw e; } else { throw e; } } }; if (f) execjs = hg.execjs;
        ...
        //非公開で非サポート
        //	finddefup closeleftab closeothertab closerighttab
        //	editmenu execcommand keyassign editcommand maketags record repeataction saverecord controlcode firstdown firstup
    }());
